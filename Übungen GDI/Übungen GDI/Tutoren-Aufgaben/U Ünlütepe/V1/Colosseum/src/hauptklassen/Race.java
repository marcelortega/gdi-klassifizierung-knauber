...
/**
 * Diese Klasse ist die Oberklasse für alle Rassen. Die Rassen Mensch, Elf und Ork erben von dieser Klasse und fügen notwendige 
 * Eigenschaften hinzu
 * @author mehmet
 *
 */
public class Race {
	...
	/**
	 * 
	 * @param race Rasse des Chars
	 * @param name	Name des Chars
	 * @param hp	Lebenspunkte des Chars
	 * @param physDamage Physischer Schaden
	 * @param magDamage	Magischer SChaden
	 * @param physDefense Physischen Verteidigung
	 * @param magDefense Magische Verteidigung
	 */
	public Race(String race, String name, int hp, int physDamage, int magDamage, int physDefense, int magDefense){
		this.race = race;
		this.name = name;
		this.hp = hp;
	 ...
	 * Gibt das aktueller Inventar in der Konsole aus
	 */
	 ...
	 * @param physDamage Neuer Basisschaden
	 */
	 ...
	 * @param magDamage Neuer magischer Basisschaden
	 */
	 ...
	 * @param physDefense Neue Physische Basisverteidigung
	 */
	 ...
	 * @param magDefense Neue Magische Basisverteidigung
	 */
	 ...
	 * @param equip Einzufügender Gegenstand
	 * @param place Platz im Inventar
	 */
	 ...
	 * @param weapon Waffe aus dem Inventar
	 */
	 ...
	 * @param armor Rüstung aus dem Inventar
	 */
	...
	/**
	 * Methode um aktuelle Waffe oder Rüstung mit einer Waffe oder Rüstung aus dem Inventar auszutauschen
	 * @param i platz des auszurüstenden Gegenstandes
	 * @param e auszurüstender Gegenstand
	 */
	public void changeEquip(int i, Equipment e){
		if(e instanceof Weapon){
			Weapon wmp = getWeapon();
			setWeapon((Weapon) e);
			equipSlots[i] = wmp;	
	 ...
	 * @param r Rasse gegen die gekämpft wird
	 */
	 ...
	 * @param phDamage Physischer Schaden
	 * @param maDamage Magischer Schaden
	...
	private int countDamage(int phDamage, int maDamage){
		...
		int damage = (phDamage-getPhysDefense())+(maDamage-getMagDefense());
		if (damage <= 0 ){
			return 1;	
		}
		else{
	 ...
	 * @param damage Zugefügter schaden
	 */
	private void setDamage(int damage, Race r){
		r.setHp(r.getHp()-damage);
		println(this.name+" Hat dem Gegner "+damage+" Schaden hinzugefügt");
		println("Gegner "+r.getName()+" hat noch "+r.getHp()+" Lebenspunkte");
	}
	...
	public void state(){
		println("********************");
		println("Character State\nRace:\t "+getRace()+"\nName:\t"+getName()+"\nLevel:\t "+getLevel()+"\nHP:\t "+getHp()+"\nPhDam:\t "+(getPhysDamage()+this.weapon.damage)+"\nPhDef:\t "+(getPhysDefense()+this.armor.defense)+"\nMaDam:\t "+getMagDamage()+"\nMaDef:\t "+getMagDefense());
		println("********************");
	}
	
	
//	public void state(Race opponent){
//		
//	}
