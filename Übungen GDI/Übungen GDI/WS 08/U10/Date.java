...
public interface Date {
	...
	/**
	 * Diese Konstante erhält beim Programmstart den aktuellen Tag im Monat.
	 */
	static final int CURRENT_DAY = GregorianCalendar.getInstance().get(GregorianCalendar.DAY_OF_MONTH);
	
	/**
	 * Diese Konstante erhält beim Programmstart den aktuellen Monat.
	 */
	static final int CURRENT_MONTH = GregorianCalendar.getInstance().get(GregorianCalendar.MONTH) + 1;
	
	/**
	 * Diese Konstante erhält beim Programmstart das aktuelle Jahr.
	 */
